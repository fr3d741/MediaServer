...
static std::string
ConvertToCommonFormat(const std::wstring& str) {

    const std::wregex number_filter(L"[0-9]{1,3}");
    auto it = std::wsregex_iterator(str.begin(), str.end(), number_filter);
...
static void
GatherEpisodesFromFS(Logging::ILogger::Ptr logger, const std::filesystem::directory_entry& dir_entry, std::map<std::string, std::wstring>& season_episodes) {
    ...
    for (auto const& season_dir : std::filesystem::directory_iterator{ dir_entry }) {
        ...
        for (auto const& file_entry : std::filesystem::directory_iterator{ season_dir }) {
            ...
            const std::wregex episode_filter(L"S[0-9]{1,3}E[0-9]{1,3}", std::regex_constants::icase);
            std::wsmatch file_match;
            ...
            if (std::regex_search(file_name, file_match, episode_filter) == false)
            {
                ...
                root.AddAttribute("file", GeneralUtilities::Convert(file_entry.path().generic_wstring()));
                logger->LogMessage(root.Dump());
                continue;
            }

            episode_count++;
            season_episodes.insert({ ConvertToCommonFormat(file_match.str()), file_entry.path().wstring() });
        }
    }
}
...
static bool 
operator<(const std::pair<const std::string, JsonNode::Ptr>& a, const std::pair<const std::string, std::wstring>& b) {
    return a.first < b.first;
}

static bool 
operator<(const std::pair<const std::string, std::wstring>& a, const std::pair<const std::string, JsonNode::Ptr>& b) {
    return a.first < b.first;
}

using InputIt1 = std::map<std::string, JsonNode::Ptr>::iterator;
using InputIt2 = std::map<std::string, std::wstring>::const_iterator;
...
GetAllSeasonConfigurations(Logging::ILogger::Ptr logger, const int nr_of_seasons, const std::string& show_id, const std::map<std::string, std::wstring>& reference) {
...
TvShow::Init() {
    ...
    {
        auto msg = std::format(L"Empty request response for {}", _title);
...
TvShow::CreateEpisodeNfos(std::map<std::string, XmlNode>& result_map) {
    ...
    std::map<std::string, std::wstring> season_episodes;
